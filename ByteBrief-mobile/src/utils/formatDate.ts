...
/**
 * iOS/Android の Hermes では Intl が存在しても内部で壊れ、
 * `Cannot read property 'prototype' of undefined` になることがあるため、
 * ネイティブでは Intl を使わず手組み表示のみにする。
 */
export function formatRelativeTime(dateString: string, locale: string = 'ja'): string {
  ...
  const useIntl = Platform.OS === 'web'

  if (useIntl && typeof Intl?.RelativeTimeFormat === 'function') {
    try {
  ...
  if (useIntl && typeof Intl?.DateTimeFormat === 'function') {
    try {
      return new Intl.DateTimeFormat(locale, {
        month: 'short',
        day: 'numeric',
